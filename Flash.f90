...
USE zTVTERMOm   
    ...
    PARAMETER (nco=64, RGAS=0.08314472d0)
        ...
        DOUBLE PRECISION, dimension(nco) :: Ttp, Ptp, Vs_Vl, C1, C2, C3, rNC           !agregado
        DOUBLE PRECISION, dimension(n) :: Ttpn, Ptpn, Vs_Vln, C1n, C2n, C3n, rNCn
        ...
        real*8, dimension(n) :: PHILOGy, PHILOGx, DLPHIT, DLPHIP, w, PHILOGw, PHILOGwu, PHILOG, U, w1, SFrac, GAMALOG
...
!                Pl = -1.0
                ...
                MTYP = 1 !para calcular las propiedades del líquido hipotético
                !y es solucion sólida
                

                call TERMO(n,MTYP,1,T,P,w1,Vx,PHILOG,DLPHIP,DLPHIT,FUGN)!para calcular el coeficiente de fugacidad en el líquido hipotético (a T, P y W)
                                        !PHILOG es el log(phi*P) de i en el HL
		        PHILOGw = PHILOG-log(P)    !PHILOGw = log(phi) de i en el HL
 		
		   
		    U = (Vs_Vln/(RGAS*Ttpn))*(C1n*(1.0D0 - Ttpn/T)+ C2n*(Ttpn/T - 1.0D0 + dlog(T/Ttpn)) + C3n*(T/(2.0D0*Ttpn) - 1.0D0 + Ttpn/(2.0D0*T)) + ((P-Ptpn)*(Ttpn/T)))
		   
            SFrac = (w1*rNC/sum(w1*rNC))
            GAMALOG = log(SFrac/w1) + 1 - (SFrac/w1) 
            
            PHILOGwu = GAMALOG + PHILOGw + U    !!!!!!!!!!!!!!!!!!!!!PHILOGwu es el log(phi) de i  en la SS NO ideal
            
            PHILOGy = PHILOGwu + dlog(P)            !PHILOGy es el ln(phi(i)*P) de la SS ideal
...
!       vmax=1.001d0   ! modified  3/3/15 (not to generate false separations with beta 0.9999...)
        ...
        do i=1,n
            if (KFACT(i)*z(i)>1)then
                ini = ini+1
                vmin(ini) = (KFACT(i)*z(i)-1.d0)/(KFACT(i)-1.d0)
